...
finish within the token's ~24 h lifetime). Separately, `ZAP_RULE_MINS` sets the **per-rule** cap
(default 5 min) — the safety net that stops any single rule hanging the scan; leave it non-zero even
when the total is unlimited. If the summary shows `[hit the …m cap]`, coverage was incomplete —
raise `ZAP_ASCAN_MINS`. With `OPENAPI_URL` set, the spec is validated **before** the scan so a bad URL is caught up front.
